-- 文件: actionNormal/AiNormalFarmerRightStartData.lua · 反编译 AI 模块（阅读用）

local AiNormalFarmerRightStartData = class("AiNormalFarmerRightStartData", AiFarmerRightStartData)

-- 中等AI·下家农民首出 · 构造函数
function AiNormalFarmerRightStartData:ctor(params)
  AiNormalFarmerRightStartData.super.ctor(self, params)
end

-- 中等AI·下家农民首出 · 出牌主入口：按局面分支决策并返回 AiSuitData（下家农民出牌）
function AiNormalFarmerRightStartData:getActionSuitData()
  self.log("=下家农民出牌")
...
-- 中等AI·下家农民首出 · 计算攻守姿态（0 防守 / 1 进攻）
function AiNormalFarmerRightStartData:getStatus()
...
-- 中等AI·下家农民首出 · 收官出牌：一手出完或走赢牌路径（下家农民出牌-收官）
function AiNormalFarmerRightStartData:getEndActionSuitData()
  self.log("==下家农民出牌-收官")
  local data = self:getData()
  local taskData = self.taskData_
  ...
  if 0 < #noTaskSuitDataArr then
    if 1 <= #noTaskSuitDataArr and noTaskSuitDataArr[1]:getCardCount() == data:getCardCount() then
      ...
      for _, suitData in ipairs(noTaskSuitDataArr) do
        ...
        if
          suitDataType == SuitType.kFourWithTwoPair
          or suitDataType == SuitType.kFourWithTwoSingle
          or suitDataType == SuitType.kFourStraight
        then
...
-- 中等AI·下家农民首出 · 协助队友收官的出牌（下家农民出牌-帮队友收官）
function AiNormalFarmerRightStartData:getHelpFriendEndActionSuitData()
...
-- 中等AI·下家农民首出 · 防守姿态出牌（下家农民出牌-防手姿态）
function AiNormalFarmerRightStartData:getPreventStatusActionSuitData()
...
-- 中等AI·下家农民首出 · 防守姿态：阻止对手收官的出牌（下家农民出牌-防守姿态-防队手收官）
function AiNormalFarmerRightStartData:getPreventStatusPreventEnemyActionSuitData()
  ...
  if 0 < #bestMismatchWinConditionArr or 0 < #bestDd then
    ...
    self:addParamsWithKey(
      "比“队友的任意牌组合中，对手的D牌组合”小的“任意牌组合中(不拆炸弹)，对手的d牌组合”",
      friendMismatchWinConditionArr
    )
    self.log(
      "===任意牌组合中(不拆炸弹)，对手的D牌组合数量："
        .. #mismatchWinConditionArr
        .. "; 对手的d牌组合数量："
        .. #matchWinConditionArr
        .. "; Dd牌组合数量："
        .. #Dd
    )
    if 0 < #mismatchWinConditionArr or 0 < #Dd then
      ...
      if not AiUtils:isAllMaxSuitData(bestSuitDataArr) then
        self.log("====最优分组中，不全是最大牌")
        local bestNotMaxSuitDataArr = AiUtils:filterNotMaxSuitDataArr(bestSuitDataArr)
        self:addParamsWithKey("最优分组的非最大牌的牌组合", bestNotMaxSuitDataArr)
        local maxLevelSuitData = AiUtils:getMaxLevelSuitData(bestNotMaxSuitDataArr)
        self:addParamsWithKey("最优分组的非最大牌的最大的牌组合", { maxLevelSuitData })
        self.log("=====【优先出的牌组合列表】=最优分组的非最大牌的最大的牌组合")
        self:setFirstSuitDataArr({ maxLevelSuitData })
        self:addResult("J402")
        return self:getBaseActionSuitData()
      else
        self.log("====最优分组中，全是最大牌")
        self:addResult("J401")
      end
...
-- 中等AI·下家农民首出 · 获取防守姿态常规出牌动作牌型data（下家农民出牌-防守姿态-常规出牌）
function AiNormalFarmerRightStartData:getPreventStatusNormalActionSuitData()
  ...
  do
    ...
    self:addParamsWithKey(
      "任意牌中，与“队友最优分组的L牌组合”同牌型的，队友的Lax牌的牌组合 E9",
      e9
    )
    self.log(
      "====任意牌中，与“队友最优分组的L牌组合”同牌型的，队友的Lax牌的牌组合。假定是E9，E9数量是："
        .. #e9
    )
    if 0 < #e9 then
      do
        self:addResult("J53")
        local e7 = data:ai_all_filterAllSmallerMaxSuitDataArr(e9)
        self:addParamsWithKey("E9中，同牌型有压制最大牌的牌组合。假定是E7牌组合 E7", e7)
        self.log("=====E9中，同牌型有压制最大牌的牌组合。假定是E7，E7数量是：" .. #e7)
        if 0 < #e7 then
      ...
      self:addParamsWithKey(
        "任意牌中，比【“队友最优分组的压制强收官牌型”的最大牌】小的牌组合 E12",
        e12
      )
      self.log(
        "====任意牌中，比【“队友最优分组的压制强收官牌型”的最大牌】小的牌组合。假定是E12，E12数量是："
          .. #e12
      )
      if 0 < #e12 then
...
-- 中等AI·下家农民首出 · 进攻姿态出牌（下家农民出牌-进攻姿态）
function AiNormalFarmerRightStartData:getAttackStatusActionSuitData()
...
-- 中等AI·下家农民首出 · 获取进攻姿态防守对手出牌动作牌型data（下家农民出牌-进攻姿态-防队手收官）
function AiNormalFarmerRightStartData:getAttackStatusPreventEnemyActionSuitData()
...
-- 中等AI·下家农民首出 · 进攻姿态：常规出牌（下家农民出牌-进攻姿态-常规出牌）
function AiNormalFarmerRightStartData:getAttackStatusNormalActionSuitData()
...
-- 中等AI·下家农民首出 · 获取extend赢牌/收官conditionsby牌型dataarr（下家农民出牌-收官扩展(四带X)）
function AiNormalFarmerRightStartData:getExtendWinConditionsBySuitDataArr(suitDataArr)
...
-- 中等AI·下家农民首出 · 剩两张牌时的特殊出牌处理（下家农民出牌-只剩两手牌）
function AiNormalFarmerRightStartData:getLeftTwoSuitData()
...
return AiNormalFarmerRightStartData
