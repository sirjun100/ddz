-- 文件: actionNormal/AiNormalFarmerRightFollowData.lua · 反编译 AI 模块（阅读用）

local AiNormalFarmerRightFollowData = class("AiNormalFarmerRightFollowData", AiFarmerRightFollowData)

-- 中等AI·下家农民跟牌 · 构造函数
function AiNormalFarmerRightFollowData:ctor(params)
  AiNormalFarmerRightFollowData.super.ctor(self, params)
end

-- 中等AI·下家农民跟牌 · 出牌主入口：按局面分支决策并返回 AiSuitData（下家农民跟牌）
function AiNormalFarmerRightFollowData:getActionSuitData()
  self.log("=下家农民跟牌")
...
-- 中等AI·下家农民跟牌 · 计算攻守姿态（0 防守 / 1 进攻）
function AiNormalFarmerRightFollowData:getStatus()
...
-- 中等AI·下家农民跟牌 · 收官出牌：一手出完或走赢牌路径（下家农民跟牌-收官）
function AiNormalFarmerRightFollowData:getEndActionSuitData()
  self.log("==下家农民跟牌-收官")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  local taskData = self.taskData_
...
-- 中等AI·下家农民跟牌 · 协助队友收官的出牌（下家农民跟牌-帮队友收官）
function AiNormalFarmerRightFollowData:getHelpFriendEndActionSuitData()
...
-- 中等AI·下家农民跟牌 · 防守姿态出牌（下家农民跟牌-防手姿态）
function AiNormalFarmerRightFollowData:getPreventStatusActionSuitData()
...
-- 中等AI·下家农民跟牌 · 防守姿态：阻止对手收官的出牌（下家农民跟牌-防守姿态-防队手收官）
function AiNormalFarmerRightFollowData:getPreventStatusPreventEnemyActionSuitData()
  ...
  do
    ...
    if 0 < #m1 then
      if not AiUtils:isAllBomb(m1) then
        ...
        local firstSuitDataArr = data:ai_all_filterBiggerNotDisBombSuitDataArr(lastSuitData)
...
-- 中等AI·下家农民跟牌 · 获取防守姿态常规出牌动作牌型data（下家农民跟牌-防守姿态-常规跟牌）
function AiNormalFarmerRightFollowData:getPreventStatusNormalActionSuitData()
  ...
  do
    ...
    if 0 < #friendBestBiggerSuitDataArr then
      ...
      if 0 < #m4 then
        local bestBiggerSuitDataArr = data:ai_bestFenzu_filterBiggerSuitDataArr(lastSuitData)
        self:addParamsWithKey("最优分组的YQ牌组合")
              ...
              if 0 < #splitBringLiuSuitDataArr then
                ...
                if 0 < #m8 then
                  ...
                  if x then
                    ...
                    local m13_max = AiUtils:getMaxLevelSuitData(tmp)
                    if m13_max then
                      local arr = {m13_max}
                      self:addParamsWithKey("M13牌组合的最大牌", arr)
                      self.log("====【优先出的牌组合列表】=M13牌组合中的最大牌")
...
-- 中等AI·下家农民跟牌 · 进攻姿态出牌（下家农民跟牌-进攻姿态）
function AiNormalFarmerRightFollowData:getAttackStatusActionSuitData()
...
-- 中等AI·下家农民跟牌 · 获取进攻姿态防守对手出牌动作牌型data（下家农民跟牌-进攻姿态-防队手收官）
function AiNormalFarmerRightFollowData:getAttackStatusPreventEnemyActionSuitData()
...
-- 中等AI·下家农民跟牌 · 进攻姿态：常规出牌（下家农民跟牌-进攻姿态-常规跟牌）
function AiNormalFarmerRightFollowData:getAttackStatusNormalActionSuitData()
...
-- 中等AI·下家农民跟牌 · 获取地主lefttwoorone牌型data（下家农民跟牌-地主报牌）
function AiNormalFarmerRightFollowData:getLandlordLeftTwoOrOneSuitData()
...
-- 中等AI·下家农民跟牌 · 获取extend赢牌/收官conditions（下家农民跟牌-收官扩展）
function AiNormalFarmerRightFollowData:getExtendWinConditions()
...
return AiNormalFarmerRightFollowData
