-- 文件: actionNormal/AiNormalLandlordStartData.lua · 反编译 AI 模块（阅读用）

local AiNormalLandlordStartData = class("AiNormalLandlordStartData", AiLandlordStartData)

-- 中等AI·地主首出 · 构造函数
function AiNormalLandlordStartData:ctor(params)
  AiNormalLandlordStartData.super.ctor(self, params)
end

-- 中等AI·地主首出 · 出牌主入口：按局面分支决策并返回 AiSuitData（地主出牌）
function AiNormalLandlordStartData:getActionSuitData()
  self.log("=地主出牌")
...
-- 中等AI·地主首出 · 判断是否need防守enemys收官
function AiNormalLandlordStartData:isNeedPreventEnemysEnd()
  return self:farmerLeftEnableWarningEnd() or self:farmerRightEnableWarningEnd()
end

-- 中等AI·地主首出 · 收官出牌：一手出完或走赢牌路径（地主出牌-收官）
function AiNormalLandlordStartData:getEndActionSuitData()
  self.log("==地主出牌-收官")
  local data = self:getData()
  local taskData = self.taskData_
  local bestSuitDataArr = data:ai_getBestSuitDataArr()
...
-- 中等AI·地主首出 · 获取防守enemys收官出牌动作牌型data（地主出牌-防对手s收官）
function AiNormalLandlordStartData:getPreventEnemysEndActionSuitData()
...
-- 中等AI·地主首出 · 获取常规出牌动作牌型data（地主出牌-常规）
function AiNormalLandlordStartData:getNormalActionSuitData()
  self.log("==地主出牌-常规")
  local data = self:getData()
  do
...
-- 中等AI·地主首出 · 获取extend赢牌/收官conditionsby牌型dataarr（地主出牌-收官扩展(四带X)）
function AiNormalLandlordStartData:getExtendWinConditionsBySuitDataArr(suitDataArr)
...
-- 中等AI·地主首出 · 剩两张牌时的特殊出牌处理（地主出牌-只剩两手牌）
function AiNormalLandlordStartData:getLeftTwoSuitData()
  self.log("==地主出牌-只剩两手牌")
  local data = self:getData()
...
return AiNormalLandlordStartData
