-- 文件: actionWorse/AiWorseLandlordFollowData.lua · 反编译 AI 模块（阅读用）

local AiWorseLandlordFollowData = class("AiWorseLandlordFollowData", AiLandlordFollowData)

-- 弱AI·地主跟牌 · 构造函数
function AiWorseLandlordFollowData:ctor(params)
  AiWorseLandlordFollowData.super.ctor(self, params)
end

-- 弱AI·地主跟牌 · 出牌主入口：按局面分支决策并返回 AiSuitData（地主跟牌）
function AiWorseLandlordFollowData:getActionSuitData()
  self.log("=地主跟牌")
...
-- 弱AI·地主跟牌 · 判断是否need防守enemys收官（判断是否需要防对手收官）
function AiWorseLandlordFollowData:isNeedPreventEnemysEnd()
...
-- 弱AI·地主跟牌 · 收官出牌：一手出完或走赢牌路径（地主跟牌-收官）
function AiWorseLandlordFollowData:getEndActionSuitData()
  self.log("==地主跟牌-收官")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
...
-- 弱AI·地主跟牌 · 获取防守enemys收官出牌动作牌型data（地主跟牌-防对手s收官）
function AiWorseLandlordFollowData:getPreventEnemysEndActionSuitData()
...
-- 弱AI·地主跟牌 · 获取常规出牌动作牌型data（地主跟牌-常规）
function AiWorseLandlordFollowData:getNormalActionSuitData()
  self.log("==地主跟牌-常规")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  do
...
return AiWorseLandlordFollowData
