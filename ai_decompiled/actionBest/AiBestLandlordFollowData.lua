-- 文件: actionBest/AiBestLandlordFollowData.lua · 反编译 AI 模块（阅读用）

local AiBestLandlordFollowData = class("AiBestLandlordFollowData", AiLandlordFollowData)

-- 强AI·地主跟牌 · 构造函数
function AiBestLandlordFollowData:ctor(params)
  AiBestLandlordFollowData.super.ctor(self, params)
end

-- 强AI·地主跟牌 · 出牌主入口：按局面分支决策并返回 AiSuitData（地主跟牌）
function AiBestLandlordFollowData:getActionSuitData()
  self.log("=地主跟牌")
...
-- 强AI·地主跟牌 · 判断是否need防守enemys收官（判断是否需要防对手收官）
function AiBestLandlordFollowData:isNeedPreventEnemysEnd()
  ...
  if farmerLeftData:getUid() == uid then
    if farmerLeftData:ai_getEnableEnd() then
      self.log("==上家农民最后出牌，且能收官了")
      return true
    end
    if farmerRightData:ai_isMatchWinCondition(lastSuitData) then
      self.log("==上家农民最后出牌，且最后出牌是下家农民d牌")
      return true
    end
  elseif farmerRightData:ai_getEnableEnd() then
    self.log("===下家农民最后出牌，且能收官了")
    return true
  end
  return false
end

-- 强AI·地主跟牌 · 收官出牌：一手出完或走赢牌路径（地主跟牌-收官）
function AiBestLandlordFollowData:getEndActionSuitData()
  self.log("==地主跟牌-收官")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  local taskData = self.taskData_
...
-- 强AI·地主跟牌 · 获取防守enemys收官出牌动作牌型data（地主跟牌-防对手s收官）
function AiBestLandlordFollowData:getPreventEnemysEndActionSuitData()
...
-- 强AI·地主跟牌 · 获取常规出牌动作牌型data（地主跟牌-常规）
function AiBestLandlordFollowData:getNormalActionSuitData()
  self.log("==地主跟牌-常规")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  local farmerLeftData = self:getFarmerLeftData()
...
-- 强AI·地主跟牌 · 获取extend赢牌/收官conditions（地主跟牌-收官扩展）
function AiBestLandlordFollowData:getExtendWinConditions()
  self.log("==地主跟牌-收官扩展")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  local bestBiggerSuitDataArr = data:ai_bestFenzu_filterBiggerSuitDataArr(lastSuitData)
...
return AiBestLandlordFollowData
