-- 文件: actionBest/AiBestFarmerLeftFollowData.lua · 反编译 AI 模块（阅读用）

local AiBestFarmerLeftFollowData = class("AiBestFarmerLeftFollowData", AiFarmerLeftFollowData)

-- 强AI·上家农民跟牌 · 构造函数
function AiBestFarmerLeftFollowData:ctor(params)
  AiBestFarmerLeftFollowData.super.ctor(self, params)
end

-- 强AI·上家农民跟牌 · 出牌主入口：按局面分支决策并返回 AiSuitData（上家农民跟牌）
function AiBestFarmerLeftFollowData:getActionSuitData()
  self.log("=上家农民跟牌")
  ...
  if self:farmerRightEnableEnd() then
...
-- 强AI·上家农民跟牌 · 计算攻守姿态（0 防守 / 1 进攻）
function AiBestFarmerLeftFollowData:getStatus()
  local data = self:getData()
...
-- 强AI·上家农民跟牌 · 判断是否need防守enemys收官（判断是否需要防对手收官）
function AiBestFarmerLeftFollowData:isNeedPreventEnemysEnd()
  self.log("=判断是否需要防对手收官")
  local enemyEnableEnd = self:landlordEnableEnd()
  if enemyEnableEnd then
    local lastSuitData = self.lastSuitData_
...
-- 强AI·上家农民跟牌 · 收官出牌：一手出完或走赢牌路径（上家农民跟牌-收官）
function AiBestFarmerLeftFollowData:getEndActionSuitData()
  self.log("==上家农民跟牌-收官")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  local taskData = self.taskData_
...
-- 强AI·上家农民跟牌 · 协助队友收官的出牌（上家农民跟牌-帮队友收官）
function AiBestFarmerLeftFollowData:getHelpFriendEndActionSuitData()
  ...
  if friendData:getUid() == uid then
    ...
    if not enemyEnableFollow or enemyIsLose then
      friendIsWarningEnd = self:farmerRightEnableWarningEnd()
      lastSuitIsMax = nil
      self.log("====队友是否报牌收官：" .. tostring(friendIsWarningEnd))
      if friendIsWarningEnd then
        self:addResult("E65")
        self.log("=====PASS")
        ...
        local n12 = friendData:ai_filterMatchWinCondition(notMaxSuitDataArr)
        self:addParamsWithKey("最优分组的非大牌YQ牌组合中，队友d牌组合的牌组合 N12", n12)
        self.log("=====最优分组的非大牌YQ牌组合中，队友d牌组合的牌组合。假定是N12，N12数量是：" .. #n12)
        if 0 < #n12 then
          self.log("==【优先出的牌组合列表】=N12牌组合")
          self:setFirstSuitDataArr(n12)
        ...
        local n13 = friendData:ai_filterMatchWinCondition(splitBringBiggerSuitDataArr)
        self:addParamsWithKey("最优分组的YQ牌组合中，队友d牌组合的牌组合 N13", n13)
        self.log("=====最优分组的YQ牌组合中，队友d牌组合的牌组合。假定是N13，N13数量是：" .. #n13)
        if 0 < #n13 then
          self.log("==【优先出的牌组合列表】=N13牌组合")
          self:setFirstSuitDataArr(n13)
          suitData = self:getBaseActionSuitData()
          ok = true
          self:addResult("E64")
          return ok, suitData
...
-- 强AI·上家农民跟牌 · 防守姿态出牌（上家农民跟牌-防手姿态）
function AiBestFarmerLeftFollowData:getPreventStatusActionSuitData()
  self.log("==上家农民跟牌-防手姿态")
  return self:getPreventStatusNormalActionSuitData()
end

-- 强AI·上家农民跟牌 · 防守姿态：阻止对手收官的出牌（上家农民跟牌-防守姿态-防队手收官）
function AiBestFarmerLeftFollowData:getPreventStatusPreventEnemyActionSuitData()
            ...
            if not AiUtils:isAllBomb(n2) then
              ...
              if 0 < #n3 then
                ...
                if not enemyEnableWarningEnd then
                  local n10 = AiUtils:filterNotMaxMaxSuitDataArr(bestBiggerSuitDataArr)
                  self:addParamsWithKey("最优分组的非大牌的YQ牌组合 N10", n10)
                  self.log("最优分组的非大牌的YQ牌组合，假定是N10，N10数量是：" .. #n10)
                  if 0 < #n10 then
                    self.log("======【优先出的牌组合列表】=N10牌组合")
                    self:setFirstSuitDataArr(n10)
                    self:addResult("E47")
                  else
                    local n11 = AiUtils:filterNotMaxMaxSuitDataArr(splitBringBiggerSuitDataArr)
                    self:addParamsWithKey("带牌组合的非大牌的YQ牌组合 N11", n11)
                    self.log("带牌组合的非大牌的YQ牌组合，假定是N11，N11数量是：" .. #n11)
                    if 0 < #n11 then
                      self.log("======【优先出的牌组合列表】=N11牌组合")
                      self:setFirstSuitDataArr(n11)
                      self:addResult("E48")
                    else
                      self:addResult("E49")
                      ...
                      if 0 < #n8 then
                        ...
                        if 0 < #n9 then
                          ...
                          if 0 < #allBiggerNotDisBombSuitDataArr then
                            ...
                            self:addParamsWithKey("任意不拆炸弹YQ牌组合的非炸弹的YQ牌组合 N15", n15)
                            self.log("带牌组合的非炸弹的YQ牌组合，假定是N15，N15数量是：" .. #n15)
                            if 0 < #n15 then
...
-- 强AI·上家农民跟牌 · 获取防守姿态常规出牌动作牌型data（上家农民跟牌-防守姿态-常规跟牌）
function AiBestFarmerLeftFollowData:getPreventStatusNormalActionSuitData()
  ...
  do
    ...
    if friendData:getUid() ~= uid then
            ...
            if 0 < #o3 then
              ...
              if 0 < #o4 then
                ...
                if x then
                  ...
                  local o5 = AiUtils:filterNotSmallerSuitDataArr(bestLiu, x)
                  self:addParamsWithKey("最优分组YQ牌组合中，L牌组合中，不比X小的牌组合 O5", o5)
                  self.log("===最最优分组YQ牌组合中，L牌组合中，不比X小的牌组合。假定是O5，O5数量是：" .. #o5)
                  if 0 < #o5 then
                    self.log("====【优先出的牌组合列表】=O5牌组合")
                    self:setFirstSuitDataArr(o5)
                    self:addResult("G4")
                  else
                    ...
                    local o6 = AiUtils:filterNotSmallerSuitDataArr(splitBringLiu, x)
                    self:addParamsWithKey("带牌牌组合YQ牌组合中，L牌组合中，不比X小的牌组合 O6", o6)
                    self.log("===带牌牌组合YQ牌组合中，L牌组合中，不比X小的牌组合。假定是O6，O6数量是：" .. #o6)
                    if 0 < #o6 then
                      self.log("====【优先出的牌组合列表】=O6牌组合")
                      self:setFirstSuitDataArr(o6)
                      self:addResult("G5")
                    else
                      local o7 = enemyData:ai_liu_filterBiggerLmaxSuitDataArrWithoutMax(allBiggerSuitDataArr)
                      self:addParamsWithKey("任意YQ牌组合中，对手的DP牌组合 O7", o7)
                      self.log("====任意YQ牌组合中，对手的DP牌组合。假定是O7，O7数量是：" .. #o7)
                      if 0 < #o7 then
                        self.log("====【优先出的牌组合列表】=O7牌组合")
                        self:setFirstSuitDataArr(o7)
                        self:addResult("G1")
                      else
      ...
      if enemyLiuKeyMap[lastSuitData:getKey()] then
        do
          self.log("=====对手有当前牌型的L牌组合")
          ...
          if 0 < #o12 then
            ...
            if 0 < #o13 then
              ...
              if x then
                ...
                local bestLiu = AiUtils:filterLiuSuitDataArr(bestNotBomb)
                self:addParamsWithKey("最优分组YQ牌组合中，L牌组合", bestLiu)
                local o14 = AiUtils:filterNotSmallerSuitDataArr(bestLiu, x)
                self:addParamsWithKey("最优分组YQ牌组合中，L牌组合中，不比X小的牌组合 O14", o14)
                self.log("===最优分组YQ牌组合中，L牌组合中，不比X小的牌组合。假定是O14，O14数量是：" .. #o14)
                if 0 < #o14 then
                  self.log("====【优先出的牌组合列表】=O14牌组合")
                  self:setFirstSuitDataArr(o14)
                  self:addResult("GD4")
                else
                  local splitBringLiu = AiUtils:filterLiuSuitDataArr(splitBringBiggerSuitDataArr)
                  self:addParamsWithKey("带牌YQ牌组合中，L牌组合", splitBringLiu)
                  local o15 = AiUtils:filterNotSmallerSuitDataArr(splitBringLiu, x)
                  self:addParamsWithKey("带牌牌组合YQ牌组合中，L牌组合中，不比X小的牌组合 O15", o15)
                  self.log("===带牌牌组合YQ牌组合中，L牌组合中，不比X小的牌组合。假定是O15，O15数量是：" .. #o15)
                  if 0 < #o15 then
                    self.log("====【优先出的牌组合列表】=O15牌组合")
                    self:setFirstSuitDataArr(o15)
                    self:addResult("GD6")
                  else
                    local o16 = enemyData:ai_liu_filterBiggerLmaxSuitDataArrWithoutMax(allBiggerSuitDataArr)
                    self:addParamsWithKey("任意YQ牌组合中，对手的DP牌组合 O16", o16)
                    self.log("====任意YQ牌组合中，对手的DP牌组合。假定是O16，O16数量是：" .. #o16)
                    if 0 < #o16 then
                      self.log("====【优先出的牌组合列表】=O16牌组合")
                      self:setFirstSuitDataArr(o16)
                      self:addResult("GD3")
                    else
        ...
        self.log("=====对手没有当前牌型的L牌组合")
        local o10 = AiUtils:filterNotBombMaxSuitDataArr(allBiggerSuitDataArr)
        self:addParamsWithKey("任意YQ牌组合中，非炸弹最大牌（抢权最大牌、压制最大牌） O10", o10)
        self.log("====任意YQ牌组合中，非炸弹最大牌（抢权最大牌、压制最大牌）。假定O10，O10数量是：" .. #o10)
        local o11 = data:ai_farmerLeft_still_filterHasFriendLmaxOrSmallerMaxSuitDataArr(o10, lastSuitData)
        self:addParamsWithKey("O10中，某个牌组合不计它后，剩余的牌 能帮队友溜牌，或者把出牌权交给队友。\nO11", o11)
        self.log("====O10中，某个牌组合不计它后，剩余的牌 能帮队友溜牌，或者把出牌权交给队友。\n假定O11，O11数量是：" .. #o11)
        if 0 < #o11 then
          self.log("=====【优先出的牌组合列表】=O11牌组合")
          self:setFirstSuitDataArr(o11)
          self:addResult("GB")
        else
          local notMaxSuitDataArr = AiUtils:filterNotMaxSuitDataArr(bestBiggerSuitDataArr)
          notMaxSuitDataArr = AiUtils:filterNotMaxMaxSuitDataArr(notMaxSuitDataArr)
          self:addParamsWithKey("最优分组的非最大牌且非大牌的YQ牌组合", notMaxSuitDataArr)
          self.log("=====【优先出的牌组合列表】=最优分组的非最大牌且非大牌的YQ牌组合")
          self:setFirstSuitDataArr(notMaxSuitDataArr)
          self:addResult("GC")
        end
      end
...
-- 强AI·上家农民跟牌 · 进攻姿态出牌（上家农民跟牌-进攻姿态）
function AiBestFarmerLeftFollowData:getAttackStatusActionSuitData()
  self.log("==上家农民跟牌-进攻姿态")
  return self:getAttackStatusNormalActionSuitData()
end

-- 强AI·上家农民跟牌 · 进攻姿态：常规出牌（上家农民跟牌-进攻姿态-常规跟牌）
function AiBestFarmerLeftFollowData:getAttackStatusNormalActionSuitData()
...
-- 强AI·上家农民跟牌 · 获取extend赢牌/收官conditions（上家农民跟牌-收官扩展）
function AiBestFarmerLeftFollowData:getExtendWinConditions()
...
return AiBestFarmerLeftFollowData
