-- 文件: actionBest/AiBestLandlordStartData.lua · 反编译 AI 模块（阅读用）

local AiBestLandlordStartData = class("AiBestLandlordStartData", AiLandlordStartData)

-- 强AI·地主首出 · 构造函数
function AiBestLandlordStartData:ctor(params)
  AiBestLandlordStartData.super.ctor(self, params)
end

-- 强AI·地主首出 · 出牌主入口：按局面分支决策并返回 AiSuitData（地主出牌）
function AiBestLandlordStartData:getActionSuitData()
  self.log("=地主出牌")
...
-- 强AI·地主首出 · 判断是否need防守enemys收官
function AiBestLandlordStartData:isNeedPreventEnemysEnd()
  return self:farmerLeftEnableEnd() or self:farmerRightEnableEnd()
end

-- 强AI·地主首出 · 收官出牌：一手出完或走赢牌路径（地主出牌-收官）
function AiBestLandlordStartData:getEndActionSuitData()
  self.log("==地主出牌-收官")
  local data = self:getData()
  local taskData = self.taskData_
  local bestSuitDataArr = data:ai_getBestSuitDataArr()
...
-- 强AI·地主首出 · 获取防守enemys收官出牌动作牌型data（地主出牌-防对手s收官）
function AiBestLandlordStartData:getPreventEnemysEndActionSuitData()
    ...
    if firstSuitDataArr == nil or #firstSuitDataArr == 0 then
      ...
      if 0 < #b10 then
        ...
        if self:landlordIsLose() then
          self.log("==== 地主必败")
          if self:farmerRightEnableWarningEnd() or self:farmerLeftEnableWarningEnd() then
            self.log("==== 任意对手报牌收官： 是")
...
-- 强AI·地主首出 · 获取常规出牌动作牌型data（地主出牌-常规）
function AiBestLandlordStartData:getNormalActionSuitData()
  self.log("==地主出牌-常规")
  local data = self:getData()
  do
      ...
      if 0 < #b3 then
        ...
        if 0 < #b4 then
          ...
          if 0 < #b8 then
            ...
            local notMaxSuitDataArr = AiUtils:filterNotMaxMaxSuitDataArr(b3)
            self:addParamsWithKey("B3中，非大牌组合", notMaxSuitDataArr)
            self.log("===B3中，非大牌组合数量是：" .. #notMaxSuitDataArr)
            if 0 < #notMaxSuitDataArr then
...
-- 强AI·地主首出 · 获取extend赢牌/收官conditions（地主出牌-收官扩展）
function AiBestLandlordStartData:getExtendWinConditions()
  self.log("==地主出牌-收官扩展")
  ...
  local ret = data:ai_landlord_still_filterFarmerLoseSuitDataArr(bestBiggerSuitDataArr)
  if 0 < #ret then
...
-- 强AI·地主首出 · 获取extend赢牌/收官conditionsby牌型dataarr（地主出牌-收官扩展(四带X)）
function AiBestLandlordStartData:getExtendWinConditionsBySuitDataArr(suitDataArr)
  self.log("==地主出牌-收官扩展(四带X)")
  local lastSuitData = self.lastSuitData_
  local data = self:getData()
  local cardTypeListdata
  ...
  if 3 <= #biggerSuitDataArr then
    ...
    local farmerRightData = self:getFarmerRightData()
    local flag = false
    for i = #biggerSuitDataArr, 1, -1 do
      local suitData = biggerSuitDataArr[i]
      if suitData:getType() ~= SuitType.kBomb then
        local winBombCount, winNotBombCount = AiUtils:getWinSuitDataNum(suitData, farmerLeftData:getAllSuitData(), farmerRightData:getAllSuitData())
        if winBombCount <= 1 and winNotBombCount == 0 then
          if cardTypeListdata == nil then
            ...
            cardTypeListdata.enemy2 = data.enemy2
          end
          cardTypeListdata:removeCardTypeWithSuitData(suitData)
          table.insert(outSuitDataArr, suitData)
    ...
    if flag then
      self.log("=====四带X组合拆成3手牌，除开炸弹，对手要不起的大牌数量：" .. #outSuitDataArr)
    else
  ...
  local ret = cardTypeListdata:ai_landlord_still_filterFarmerLoseSuitDataArr(biggerSuitDataArr)
  if 0 < #ret then
    if 0 < #outSuitDataArr then
...
-- 强AI·地主首出 · 检查liu牌型判断是否all单张（地主-判断是否有溜牌且全是单张）
function AiBestLandlordStartData:checkLiuSuitIsAllSingle()
  self.log("==地主-判断是否有溜牌且全是单张")
  local data = self:getData()
  local farmerLeftData = self:getFarmerLeftData()
  local farmerRightData = self:getFarmerRightData()
  local winConditions = data:ai_getWinConditions()
  ...
  local farmerLeftBestFenzu = farmerLeftData:ai_getBestFenzu()
  local farmerLeftBestFenzuInfo = farmerLeftBestFenzu:getBestFenzuInfo()
  local farmerLeftSuitDataArr = farmerLeftBestFenzuInfo.suitDataArr
  self:addParamsWithKey("上家农民最优分组", farmerLeftSuitDataArr)
  for _, suitData in ipairs(farmerLeftSuitDataArr) do
    if suitData:getType() ~= SuitType.kSingle then
      self.log("===上家农民最优分组不全是单张")
      return false
    end
  end
  local farmerRightBestFenzu = farmerRightData:ai_getBestFenzu()
  local farmerRightBestFenzuInfo = farmerRightBestFenzu:getBestFenzuInfo()
  local farmerRightSuitDataArr = farmerRightBestFenzuInfo.suitDataArr
  self:addParamsWithKey("下家农民最优分组", farmerRightSuitDataArr)
  for _, suitData in ipairs(farmerRightSuitDataArr) do
    if suitData:getType() ~= SuitType.kSingle then
      self.log("===下家农民最优分组不全是单张")
      return false
    end
  end
  if #liuSuitDataArr <= #farmerLeftSuitDataArr and #liuSuitDataArr <= #farmerRightSuitDataArr then
    self.log("===溜牌数量比农民单张数量少，能赢")
    return false
  end
  return true
end

-- 强AI·地主首出 · 是否可单张收官（地主出牌-判断是否能单张收官）
function AiBestLandlordStartData:enableSingleEnd()
  self.log("==地主出牌-判断是否能单张收官")
  local data = self:getData()
  local farmerLeftData = self:getFarmerLeftData()
  local farmerRightData = self:getFarmerRightData()
  local liuSuitDataArr = data:ai_liu_getLiuSuitDataArr()
  ...
  for _, enemySuitData in ipairs(farmerLeftSuitDataArr) do
  ...
  if enemyBestFenzuWinCount >= #farmerLeftSuitDataArr then
    self.log("===比上家农民最小的牌小的溜牌数量超过对手手牌数，则不能收官")
    return false
  end
  local farmerRightBestFenzu = farmerRightData:ai_getBestFenzu()
  local farmerRightBestFenzuInfo = farmerRightBestFenzu:getBestFenzuInfo()
  local farmerRightSuitDataArr = farmerRightBestFenzuInfo.suitDataArr
  enemyMinSuitData = nil
  for _, enemySuitData in ipairs(farmerRightSuitDataArr) do
    if enemyMinSuitData == nil then
      enemyMinSuitData = enemySuitData
    elseif enemySuitData:getLevel() < enemyMinSuitData:getLevel() then
      enemyMinSuitData = enemySuitData
  ...
  enemyBestFenzuWinCount = 0
  for _, suitData in ipairs(liuSuitDataArr) do
    if enemyMinSuitData:getLevel() > suitData:getLevel() then
      enemyBestFenzuWinCount = enemyBestFenzuWinCount + 1
    end
  end
  if enemyBestFenzuWinCount >= #farmerRightSuitDataArr then
    self.log("===比下家农民最小的牌小的溜牌数量超过对手手牌数，则不能收官")
    return false
  end
  return true
end

-- 强AI·地主首出 · 获取单张收官出牌动作牌型data（地主出牌-收官-单张出牌）
function AiBestLandlordStartData:getSingleEndActionSuitData()
  self.log("==地主出牌-收官-单张出牌")
  local data = self:getData()
  local liuSuitDataArr = data:ai_liu_getLiuSuitDataArr()
...
-- 强AI·地主首出 · 剩两张牌时的特殊出牌处理（地主出牌-只剩两手牌）
function AiBestLandlordStartData:getLeftTwoSuitData()
  self.log("==地主出牌-只剩两手牌")
  local data = self:getData()
...
return AiBestLandlordStartData
